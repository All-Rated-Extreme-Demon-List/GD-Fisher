...
module.exports = {
    ...
    async execute(interaction) {
        const {db, cache, sequelize} = require('../index.js');
        const list = interaction.options.getString('list');
        ...
        if (user.id === targetUser.id) {
            return await interaction.reply({content: ':x: You cannot trade with yourself', ephemeral: true});
        }

        if (levelToGive === levelToGet) {
            return await interaction.reply({content: ':x: You cannot trade the same level', ephemeral: true});
        }
        ...
        if (!userFishedList.includes(levelToGive)) {
        ...
        try {
            ...
            const disabledRow = new ActionRowBuilder().addComponents(confirmTrade.setDisabled(true), cancelTrade.setDisabled(true));
            await tradeConfirmation.update({ components: [disabledRow] });
            ...
            if (tradeConfirmation.customId === 'confirmTrade') {
                ...
                try {
                    ...
                    // Reload fresh user data with lock
                    const [userData, targetData] = await Promise.all([
                        db[list].findOne({ where: { user: user.id }, lock: transaction.LOCK.UPDATE, transaction: transaction }),
                        db[list].findOne({ where: { user: targetUser.id }, lock: transaction.LOCK.UPDATE, transaction: transaction })
                    ]);
                    ...
                    // Re-check if users still have the levels
                    ...
                    if (userLevelIndex === -1 || targetLevelIndex === -1) {
                        await transaction.rollback();
                        return await interaction.editReply({components: [], embeds:[new EmbedBuilder().setDescription(`${tradeText}\n:x: Trade failed, one of the levels is no longer available`).setColor("Red")]});
                    }

                    // Removing levels
                    if (--userFishedListFrequency[userLevelIndex] === 0) {
                        userFishedList.splice(userLevelIndex, 1);
                        userFishedListFrequency.splice(userLevelIndex, 1);
                    }
                    if (--targetFishedListFrequency[targetLevelIndex] === 0) {
                    ...
                    await Promise.all([
                        db[list].update(
                            { fished_list: JSON.stringify(userFishedList), fished_list_frequency: JSON.stringify(userFishedListFrequency) },
                            { where: { user: user.id }, transaction }
                        ),
                        db[list].update(
                            { fished_list: JSON.stringify(targetFishedList), fished_list_frequency: JSON.stringify(targetFishedListFrequency) },
                    ...
                    await transaction.rollback();
                    return await interaction.editReply({components: [], embeds:[new EmbedBuilder().setDescription(`${tradeText}\n:x: An error occurred, please try again`).setColor("Red")]});
                }
